...
async def generate_outline_and_script(
    llm: LLM,
    query: str,
    ...
    out = await llm.generate(
        prompt,
        system=SYSTEM_SCRIPT_GENERATION,
        task="script",
    )
    ...
    if _script_needs_retry(script):
        retry_prompt = (
            prompt
            + "\n\nДополнительное требование: сценарий получился слишком кратким. "
              "Сделай его более насыщенным фактами и развернутым, но без выдумки. "
              "Верни строго один JSON-объект без текста до или после JSON."
        )

        try:
            retry_out = await llm.generate(
                retry_prompt,
                system=SYSTEM_SCRIPT_GENERATION,
                task="script",
            )
            ...
            # Retry — это улучшение качества, а не обязательный шаг.
            # Если retry вернул кривой JSON, не роняем весь pipeline.
            pass
